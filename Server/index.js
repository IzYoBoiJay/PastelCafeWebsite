...
app.get("/total", async(request, response) => {
    ...
    try {
        ...
        const cartTotal = await pool.query("SELECT SUM(price) FROM ordercontainsitems NATURAL JOIN menuItem WHERE orderNum = 1 GROUP BY price");
        
        response.json(cartTotal.rows[0]);
